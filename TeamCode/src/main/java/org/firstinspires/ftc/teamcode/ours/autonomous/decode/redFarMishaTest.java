...
public class redFarMishaTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Pose2d startPose = new Pose2d(-60, -10, Math.toRadians(0));
        Pose2d shootingPose = new Pose2d(-56, -8, Math.toRadians(-20));
        Pose2d farRowPose = new Pose2d(-35.5, -30, Math.toRadians(90));
        Pose2d middleRowPose = new Pose2d(-12, -30, Math.toRadians(90));
        ...
        Action seq1 = drive.actionBuilder(startPose)
                .strafeToLinearHeading(new Vector2d(-56, -7), Math.toRadians(-29))
                .waitSeconds(1)
                .build();

        // Go to far row
        Action seq12 = drive.actionBuilder(shootingPose)
                .strafeToLinearHeading(farRow, Math.toRadians(90))
                .build();
        ...
        // Intake forward 2
        Action seqIntakeForward1_2 = drive.actionBuilder(new Pose2d(-35.5, -35, Math.toRadians(90)))
                .strafeToLinearHeading(new Vector2d(-35.5, -40), Math.toRadians(90))
                .build();

        // Go back to shooting position
        Action seq2 = drive.actionBuilder(new Pose2d(-35.5, -40, Math.toRadians(90)))
                .strafeToLinearHeading(shootingPos, Math.toRadians(-20))
                .build();

        // Go to middle row
        Action seq3 = drive.actionBuilder(shootingPose)
                .strafeToLinearHeading(middleRow, Math.toRadians(90))
                .build();
        ...
        Action seq4 = drive.actionBuilder(new Pose2d(-12, -52, Math.toRadians(90)))
                .strafeToLinearHeading(shootingPos, Math.toRadians(-20))
                .build();


        while (!isStarted() && !isStopRequested()) {
            telemetry.addLine("RedFarAuto ready");
            telemetry.update();
        }
